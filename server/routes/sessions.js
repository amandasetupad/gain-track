...
export function sessionsRouter(db) {
  ...
  router.post('/', async (req, res) => {
    ...
    const workout = await db.prepare('SELECT id FROM workouts WHERE id = ? AND user_id = ?').get(workoutId, req.userId);
  ...
  router.get('/history/exercise-names', async (req, res) => {
  ...
  router.get('/history/by-name', async (req, res) => {
